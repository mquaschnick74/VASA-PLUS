...
function resolveLayerPath(filename: string): string {
  // Try from process.cwd() first (Replit/production), then relative to __dirname
  const candidates = [
...
// Layer 1 uses the project-level source file
...
export function assembleProfileBlock(
  ...
  if (!profile || isFirstSession) {
    return `[CLIENT CONTEXT — ${firstName}] Working stage: Pointed Origin — first session, Prescripting beginning. Register pattern: No prior session data. Known CVDC: CVDC not yet identified — in active Prescripting. Patterns: No patterns recorded. Prior significant moments: No prior sessions.`;
  }

  const block = `[CLIENT CONTEXT — ${firstName}] ${assembleCSSStage(profile)} ${assembleRegisterPattern(profile)} Known CVDC: CVDC not yet identified — in active Prescripting. ${assemblePatterns(profile)} Prior significant moments: Not yet available — session finalizer build pending.`;
