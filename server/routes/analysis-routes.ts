 ...
 *
 * Query params:
 *   - type?: 'session_summary' | 'intent_analysis' | 'concept_insights' (optional filter)
...
router.get('/list', requireAuth, async (req: AuthRequest, res) => {
  try {
    ...
    const analysisType = type as AnalysisType | undefined;
    const limitNum = parseInt(limit as string) || 10;

    // Don't allow filtering by pca_master (agent-only)
    if (analysisType === 'pca_master') {
      return res.status(400).json({
        error: 'Invalid filter',
        message: 'Cannot filter by pca_master - this analysis type is not viewable'
      });
    }

    const service = await getAnalysisService();
    const analyses = await service.getUserAnalyses(userId, analysisType, limitNum);
 ...
 */
router.get('/:analysisId', requireAuth, async (req: AuthRequest, res) => {
  try {
    const authUserEmail = req.user?.email;
    if (!authUserEmail) {
      return res.status(401).json({ error: 'User not authenticated' });
    ...
    if (error.message.includes('not viewable')) {
      return res.status(403).json({
        error: 'Analysis not viewable',
        message: 'This analysis type is only used internally and cannot be viewed.'
      });
    }
